...
class EvaluationDataset(Dataset):
    ...
    一个用于评估阶段的PyTorch Dataset。
    它负责加载指定的数据集，并使用从配置文件中传入的字段名进行解析。
    """
    ...
    def __init__(self, dataset_config: Dict, prompt_constructor: PromptConstructor, tokenizer: AutoTokenizer):
        ...
        # [FIX] 从传入的 specific dataset_config 中获取字段名
    ...
    def _load_data(self) -> List[Dict]:
        ...
        path = self.config['path']
        print(f"Loading evaluation dataset: {name} from {path}")
        print(f"Using keys -> question: '{self.question_key}', answer: '{self.answer_key}'")
        ...
        try:
            ...
            try:
                if path.endswith((".jsonl", ".json")):
                    ...
                    raise ValueError(f"Unsupported local file format for dataset '{name}': {path}.")
            except Exception as e:
                raise IOError(f"Failed to load local dataset file '{path}'. Error: {e}")
        except Exception as e:
            raise IOError(f"Failed to load dataset '{name}' from HuggingFace Hub or local path '{path}'. Error: {e}")
        ...
        processed_data = []
        for item in dataset:
            # [FIX] 使用配置定义的键，并用 .get() 增加安全性
            problem = item.get(self.question_key)
            answer_raw = item.get(self.answer_key)

            if problem is None or answer_raw is None:
                print(
                    f"Warning: Skipping item in '{name}' with missing key '{self.question_key}' or '{self.answer_key}': {item}")
                continue
